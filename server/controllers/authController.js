...
const createAndSendToken = function (user, statusCode, res) {
  ...
  const cookieOption = {
    expiresIn: new Date(Date.now() + process.env.JWT_COOKIE_EXPIRES_IN * 24 * 60 * 60 * 24),
    // secure: true,
    httpOnly: true,
  };
...
exports.login = catchAsync(async (req, res, next) => {
  ...
  if (!email || !password) return next(new AppError('enter email and password', 400));
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!freshUser) return next(new AppError('user does not exist log in again!', 401));

  // 4)check user changed password after jwt was issued
  if (!freshUser.changedPasswordAfter()) return next(new AppError('the password has been changed please login again', 401));
...
exports.restrictTo = function (...roles) {
  return (req, res, next) => {
    if (!roles.includes(req.user.role)) return next(new AppError('you not have permision to perform this action', 403));
    else next();
  };
};
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) return next(new AppError('token is invalid or has been expired', 400));
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (newPassword != passwordConfirm) return next(new AppError('new password and password confirm are not same', 403));

  // check password confirm is same to the database password
  if (!(await user.correctPassword(currentPassword, user.password))) return next(new AppError('invalid current password', 400));
